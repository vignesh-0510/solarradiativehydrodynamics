...
parser.add_argument('-p', '--postfix', type=str, default='baseline',
...
gif_filename = f'prediction_intensity_{postfix}.gif' if prediction_data else 'input_intensity.gif'
img_prefix = 'image'
np_filename = f'intensity_{postfix}.npz' if prediction_data else 'intensity_input.npz'
...
if prediction_data:
    ...
    moved_par = np.moveaxis(par,[0,1],[2,3]).reshape(-1, npar)
    lr = moved_par[:,3]
    lt = moved_par[:,4]
...
create_gif_from_array(intens, '/app/gifs', gif_filename, duration=200)
np.savez_compressed(os.path.join('/app/intensity', np_filename), intensity = intens)

for i in range(0, nt):
    hdu = fits.PrimaryHDU(intens[:,:,i])
    hdu.writeto(f'{image_dir}/{img_prefix}{i}.fits', overwrite = True)
    plot.image(intens[:,:,i], f'{image_dir}/{img_prefix}{i}', save = True)
